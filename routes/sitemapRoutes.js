...
router.get('/sitemap.xml', async (req, res) => {
    try {
        ...
        const [stores, blogs] = await Promise.all([
            Store.find({}).select('slug updatedAt').lean(),
...
`;
        ...
        res.set('Cache-Control', 'public, max-age=300, s-maxage=3600, stale-while-revalidate=600');
        res.send(sitemap);
    } catch (error) {
